...
/**
 * 固定タイトルパターン（運用方針 v2: 2026-06 検索意図最適化版）
 *   平均年収あり: 「{社名}の平均年収{XX}万円は本当？30代・40代の手取りまで【{YYYY}年最新】」
 *   平均年収なし: 「{社名}の年収はいくら？30代・40代の手取りまで有報から解説【{YYYY}年最新】」
 *
 * v1 比の意図的な変更:
 *   - 末尾「！」(煽り) → 「？」(検証姿勢) — YMYL/E-E-A-T の信頼性を底上げ
 *   - 「年代別年収や役職別年収・手取り推計情報」(キーワード羅列・冗長) →
 *     「30代・40代の手取り」 (検索者の本音語・具体年代)
 *   - 「は本当？」で「年収ガイド」「OpenWork」等の競合の数値と差があるかを
 *     確かめたい検索者を引き付ける
 *   - 文字数は同程度。SERP 32 字目安は超えるが、視認できる先頭に CTR ドライバが集約
 *
 * - パターンは固定。SERP 表示枠（全角 32 字目安）は超過しうるが、固定構造を優先する方針
 * - 年（{YYYY}）は実行時点の今年（記事更新タイミングを示す鮮度シグナル）
...
export function buildSalaryTitle(ctx: SalaryArticleContext): SalaryTitleResult {
  ...
  const buildWithName = (n: string): string => {
    if (avgMan != null) {
      return `${n}の平均年収${avgMan}万円は本当？30代・40代の手取りまで【${year}年最新】`;
    }
    return `${n}の年収はいくら？30代・40代の手取りまで有報から解説【${year}年最新】`;
  };
