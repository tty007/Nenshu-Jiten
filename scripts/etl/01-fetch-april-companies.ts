/**
 * 01: 2026年4月に有報を提出した事業会社（formCode=030000）の一覧を取得し、
 *     EDINETコード一覧と突き合わせて industry name → code を解決して
 *     scripts/etl/data/april-companies.json に保存する。
 */
...
async function fetchEdinetMaster(): Promise<Map<string, { name: string; industry: string; secCode: string }>> {
  ...
  const map = new Map<string, { name: string; industry: string; secCode: string }>();
...
async function main() {
  ...
  console.log("[1/3] EDINET 4月分の文書一覧を収集中...");
  const all: Array<{
    docID: string;
    edinetCode: string;
    secCode: string | null;
  ...
  for (const date of dateRange("2026-04-01", "2026-04-26")) {
  ...
  const outPath = path.join(OUT_DIR, "april-companies.json");
