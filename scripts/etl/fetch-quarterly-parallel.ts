...
const LABEL = arg("label") ?? "2025q4";
const TARGET_START = arg("start") ?? "2025-10-01";
const TARGET_END = arg("end") ?? "2025-12-31";
const HISTORICAL_START =
  arg("hist-start") ?? `${Number(TARGET_START.slice(0, 4)) - 5}-01-01`;
const HISTORICAL_END = arg("hist-end") ?? shiftDate(TARGET_START, -1);
const CONCURRENCY = arg("concurrency") ? Number(arg("concurrency")) : 10;

const TARGET_FILE = path.join(DATA_DIR, `target-companies-${LABEL}.json`);
const HISTORICAL_FILE = path.join(DATA_DIR, `historical-docs-${LABEL}.json`);
const FAILURES_FILE = path.join(DATA_DIR, `failures-${LABEL}.json`);
...
async function phase3Process(
  ...
  let work = docs;
  if (arg("force") !== "true") {
    // Chunk through our work-list doc_ids — a bare .select() is capped at 1000 rows by PostgREST
    ...
    for (let i = 0; i < allIds.length; i += 500) {
      ...
      if (error) throw error;
      for (const r of existing ?? []) skipSet.add(r.doc_id);
    }
